...
```{r}
draw_biplot(bread_data, point_labels = TRUE, point_size = 2, label_size = 3)
```
...
**Bagels Biplot**
Below is the bi-plot of brands for bagels.
...
```{r}
#Save data for interactive plots
...
# Select top 30 rows based on Fiber
top_fiber_data <- vege_fruit_data |>
  rename(Fiber = `Fiber, total dietary G`) |> 
  arrange(desc(Fiber)) |>
  slice_head(n = 10) |>  # Select the top 30 rows
  mutate( 
    description = case_when(
      description == "ACTUALLY SPICY CRUNCHY LENTILS SUPERFOOD SNACK, ACTUALLY SPICY" ~ "SPICY CRUNCHY LENTILS SUPERFOOD SNACK",
      # description == "ACTUALLY SPICY CRUNCHY LENTILS SUPERFOOD SNACK, ACTUALLY SPICY" ~ "SPICY CRUNCHY LENTILS SUPERFOOD SNACK",
      # description == "100% BABY KALE" ~ "BABY KALE",
      # description == "(GREEN) HOUSE CAESAR CRUNCHY GREEN LEAF, ROMAINE LETTUCE AND ARUGULA, SHREDDED SHARP PARMESAN CHEESE, AND CRISPY GARLIC PARMESAN CROUTONS SALAD KIT" ~ "REVOL GREENS SALAD KIT",
      # description == "A SUPERFOOD SALAD WITH TENDER, TASTY BABY KALE" ~ "BABY KALE",
      # description == "50/50 BLEND BABY SPINACH & SPRING MIX, 50/50 BLEND" ~ "BlEND BABY SPINACH & SPRING MIX",
      # description == "50/50 MIX SALAD" ~ "FRESH EXPRESS MIX SALAD",
      # description == "100% RUSSET JULIENNE POTATOES IN A CREAMY CHEESE SAUCE" ~ "RUSSET JULIENNE POTATOES IN A CREAMY CHEESE SAUCE",
      TRUE ~ description  # Keep the original description for all other rows
    )
  )
    
rows_to_delete <- c("50/50 BLEND A BLEND OF SPRING MIX & SPINACH, 50/50 BLEND",
                    "A SUPERFOOD SALAD WITH TENDER, TASTY BABY KALE",
                    "BlEND BABY SPINACH & SPRING MIX",
                    "50-50 BABY SPINACH, BABY LETTUCE, BABY GREENS, RADICCHIO, 50-50",
                    "50-50 BABY SPINACH, BABY LETTUCES, BABY GREENS, RADICCHIO, 50-50")

# Draw Cleveland dot plot
ggplot(top_fiber_data, aes(x = Fiber, y = fct_reorder(description, Fiber))) +
  geom_point(color = 'blue') +
  ggtitle("Cleveland Dot Plot for Top 30 Fiber-Rich Vegetables or Fruits") +
  xlab("Fiber g/100g") +
  ylab("") +
  theme_linedraw() +
  theme(
    axis.text.y = element_text(size = 8)  # Adjust Y-axis label size for better readability
  )+
...
# # Select top 30 rows based on Calcium
# rows_to_delete <- c("50/50 BLEND A BLEND OF SPRING MIX & SPINACH, 50/50 BLEND", 
#                     "A SUPERFOOD SALAD WITH TENDER, TASTY BABY KALE",
#                     "BlEND BABY SPINACH & SPRING MIX",
#                     "50-50 BABY SPINACH, BABY LETTUCE, BABY GREENS, RADICCHIO, 50-50",
#                     "50-50 BABY SPINACH, BABY LETTUCES, BABY GREENS, RADICCHIO, 50-50")
# 
# top_cal_data <- vege_fruit_data |>
#   rename(Calcium = `Calcium, Ca MG`) |> 
#   arrange(desc(Calcium)) |>
#   slice_head(n = 40)  |> 
#   mutate(
#     description = case_when(
#       description == "100% NATURAL ORGANIC CHIA SEEDS" ~ "ORGANIC CHIA SEEDS",
#       description == "ACTUALLY SPICY CRUNCHY LENTILS SUPERFOOD SNACK, ACTUALLY SPICY" ~ "SPICY CRUNCHY LENTILS SUPERFOOD SNACK",
#       description == "100% BABY KALE" ~ "BABY KALE",
#       description == "(GREEN) HOUSE CAESAR CRUNCHY GREEN LEAF, ROMAINE LETTUCE AND ARUGULA, SHREDDED SHARP PARMESAN CHEESE, AND CRISPY GARLIC PARMESAN CROUTONS SALAD KIT" ~ "REVOL GREENS SALAD KIT",
#       description == "A SUPERFOOD SALAD WITH TENDER, TASTY BABY KALE" ~ "BABY KALE",
...
#     )
#   ) |> 
#   filter(!description %in% rows_to_delete)
# 
# # Draw Cleveland dot plot
# ggplot(top_cal_data, aes(x = Calcium, y = fct_reorder(description, Calcium))) +
#   geom_point(color = "blue") +
#   ggtitle("Cleveland Dot Plot for Top 30 Calcium-Rich Vegetables or Fruits") +
#   xlab("Calcium mg/100g") +
#   ylab("") +
#   theme_linedraw() +
#   theme(
#     axis.text.y = element_text(size = 8)  # Adjust Y-axis label size for better readability
...
# Select top 30 rows based on Vitamin C, total ascorbic acid MG
# rows_to_delete <- c("50/50 BLEND A BLEND OF SPRING MIX & SPINACH, 50/50 BLEND", 
#                     "A SUPERFOOD SALAD WITH TENDER, TASTY BABY KALE",
#                     "BlEND BABY SPINACH & SPRING MIX",
#                     "50-50 BABY SPINACH, BABY LETTUCE, BABY GREENS, RADICCHIO, 50-50",
...
# top_vc_data <- vege_fruit_data |>
#   rename(Vitamin_C = `Vitamin C, total ascorbic acid MG`) |> 
#   arrange(desc(Vitamin_C)) |>
#   slice_head(n = 15) |> 
#    filter(!is.na(Vitamin_C))
#   # mutate(
#   #   description = case_when(
#   #     description == "100% NATURAL ORGANIC CHIA SEEDS" ~ "ORGANIC CHIA SEEDS",
#   #     description == "ACTUALLY SPICY CRUNCHY LENTILS SUPERFOOD SNACK, ACTUALLY SPICY" ~ "SPICY CRUNCHY LENTILS SUPERFOOD SNACK",
...
# 
# # Draw Cleveland dot plot
# ggplot(top_vc_data, aes(x = Vitamin_C, y = fct_reorder(description, Vitamin_C))) +
#   geom_point(color = "blue") +
#   ggtitle("Cleveland Dot Plot for Top 30 Vitamin_C-Rich Vegetables or Fruits") +
#   xlab("Vitamin_C mg/100g") +
#   ylab("") +
#   theme_linedraw() +
#   theme(
#     axis.text.y = element_text(size = 8)  # Adjust Y-axis label size for better readability
...
```{r}
snack_bi <- latest_data[latest_data$foodCategory %in% "Cakes, Cupcakes, Snack Cakes", ] |> 
  select(
    description, 
    packageWeight, 
    brandName,
...
snack_bi_cleaned <- align_weight_scale(snack_bi, food_type = "solid")
```


```{r}
draw_biplot(snack_bi_cleaned |> select(-description), fix_sign = TRUE)
```
